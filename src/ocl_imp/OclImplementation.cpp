...
TensorF* OclImplementation::Conv2D(WorkScheduler scheduler, TensorF* inputTn, TensorF* weights, TensorF* biases, int overrideDim2){
    ...
    */
    ...
    error |= clSetKernelArg(kernelObject->kernel, 3 , sizeof(cl_float)*OCL_BOTTLENCK_BLOCKSIZE , NULL);
    error |= clSetKernelArg(kernelObject->kernel, 4 , sizeof(cl_uint) , (void*)&B);
    error |= clSetKernelArg(kernelObject->kernel, 5 , sizeof(cl_uint) , (void*)&N);
    error |= clSetKernelArg(kernelObject->kernel, 6 , sizeof(cl_uint) , (void*)&K);
    error |= clSetKernelArg(kernelObject->kernel, 7 , sizeof(cl_uint) , (void*)&D);
    error |= clSetKernelArg(kernelObject->kernel, 8 , sizeof(cl_uint) , (void*)&chOut);

    if(error != CL_SUCCESS) cout<<getErrorString(error)<<endl;
    assert(error==0);

    cl_event exeEvt;
    size_t localThreads[]  = {OCL_BOTTLENCK_BLOCKSIZE};
    size_t globalThreads[] = {B*N*K*OCL_BOTTLENCK_BLOCKSIZE};
