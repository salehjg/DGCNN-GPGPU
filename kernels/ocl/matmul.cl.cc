...
// C = AB
kernel void kernel_batch_matmul(
        global const float *  __restrict__ matA,
        global const float *  __restrict__ matB,
        global float *  __restrict__ matC,
        local float * smem,
        const int dim0,
        const int dim1A,
        const int dim2A,
        ...
        const int dim1C,
        const int dim2C,
        const unsigned int worksize_x,
        const unsigned int worksize_y,
        const unsigned int worksize_z
        ){
    unsigned int gtid_x = get_group_id(0) * get_local_size(0) + get_local_id(0);
    unsigned int gtid_y = get_group_id(1) * get_local_size(1) + get_local_id(1);
    unsigned int gtid_z = get_group_id(2) * get_local_size(2) + get_local_id(2);
    ...
    { ///TODO: Branch Divergance and big delays!! beacause of sync threads!

        const unsigned int len_subA = BLOCK_SIZE_Y * dim2A, len_subB =
                BLOCK_SIZE_X * dim1B; //len of sub matrices of A and B.
        const unsigned long
